...
final class ColophonTest extends TestCase {
	...
	public function test_never_collapsed_shows(): void {
		$this->assertFalse( gwc_pp_colophon_snoozed( 0, 1_800_000_000 ) );
	}

	public function test_collapsed_just_now_stays_folded(): void {
		$now = 1_800_000_000;

		$this->assertTrue( gwc_pp_colophon_snoozed( $now, $now ) );
		$this->assertTrue( gwc_pp_colophon_snoozed( $now - DAY_IN_SECONDS, $now ) );
	}

	public function test_it_comes_back_after_thirty_days(): void {
		$now = 1_800_000_000;

		$this->assertTrue( gwc_pp_colophon_snoozed( $now - ( 29 * DAY_IN_SECONDS ), $now ) );
		$this->assertFalse( gwc_pp_colophon_snoozed( $now - ( 30 * DAY_IN_SECONDS ), $now ) );
		$this->assertFalse( gwc_pp_colophon_snoozed( $now - ( 400 * DAY_IN_SECONDS ), $now ) );
	}
	...
	public function test_a_timestamp_from_the_future_does_not_hide_it_for_years(): void {
		...
		$this->assertFalse(
			gwc_pp_colophon_snoozed( $now + ( 365 * DAY_IN_SECONDS ), $now ),
	...
	public function test_a_negative_timestamp_shows(): void {
		$this->assertFalse( gwc_pp_colophon_snoozed( -1, 1_800_000_000 ) );
	}
}
