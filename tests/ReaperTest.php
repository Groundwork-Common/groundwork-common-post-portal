 ...
 *
 * This is the third instance of that one bug. gwc_pp_every_pending_post_id() and
 * gwc_pp_reviewable_post_ids() both carry long comments about a newest-first cap
...
final class ReaperTest extends TestCase {
	...
	protected function setUp(): void {
		...
		foreach ( array( self::OLD_ORPHAN, self::OLD_CLAIMED, self::YOUNG, self::UNFLAGGED ) as $id ) {
			gwc_pp_test_post( $id, 'attachment', 'inherit' );
		}

		$long_ago = time() - GWC_PP_ORPHAN_AGE - DAY_IN_SECONDS;

		update_post_meta( self::OLD_ORPHAN, GWC_PP_PENDING_ATTACHMENT_META, $long_ago );
		update_post_meta( self::OLD_CLAIMED, GWC_PP_PENDING_ATTACHMENT_META, $long_ago );
		update_post_meta( self::YOUNG, GWC_PP_PENDING_ATTACHMENT_META, time() - HOUR_IN_SECONDS );
	...
	public function test_the_sweep_asks_for_the_oldest_uploads_first(): void {
		gwc_pp_reap_orphan_uploads();

		$queries = gwc_pp_test_queries_for( 'attachment' );
	...
	public function test_the_sweep_is_still_bounded(): void {
		...
		$this->assertSame( GWC_PP_PENDING_ATTACHMENT_META, $queries[0]['meta_key'] ?? null );
	}

	/* ── What it actually deletes ────────────────────────────────────────── */

	public function test_an_aged_unclaimed_upload_is_deleted(): void {
		gwc_pp_test_queue_posts( 'attachment', array( self::OLD_ORPHAN ) );

		$this->assertSame( 1, gwc_pp_reap_orphan_uploads() );
		$this->assertSame( array( self::OLD_ORPHAN ), $GLOBALS['gwc_pp_test']['deleted_attachments'] );
	}

	public function test_an_upload_a_pending_changeset_still_names_is_kept(): void {
		$post = 60;
		gwc_pp_test_post( $post, 'clinic', 'publish' );
		...
		gwc_pp_test_queue_posts( 'attachment', array( self::OLD_CLAIMED ) );
		gwc_pp_test_queue_posts( 'clinic', array( $post ) );

		$this->assertSame( 0, gwc_pp_reap_orphan_uploads() );
		$this->assertSame( array(), $GLOBALS['gwc_pp_test']['deleted_attachments'] );
	}

	public function test_a_recent_upload_is_left_alone(): void {
		gwc_pp_test_queue_posts( 'attachment', array( self::YOUNG ) );
	...
	public function test_an_upload_with_no_pending_flag_is_never_touched(): void {
		...
		gwc_pp_test_queue_posts( 'attachment', array( self::UNFLAGGED ) );
