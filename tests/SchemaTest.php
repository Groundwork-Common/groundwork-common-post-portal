...
final class SchemaTest extends TestCase {
	...
	public function test_a_leading_underscore_is_stripped(): void {
		$this->assertSame(
			'phone',
			gwc_pp_sanitize_field_key( '_phone' ),
	...
	public function test_a_user_key_can_never_collide_with_a_synthetic_one(): void {
		$this->assertSame( 'title', gwc_pp_sanitize_field_key( '__title' ) );
		$this->assertNotSame( '__title', gwc_pp_sanitize_field_key( '__title' ) );
	}

	public function test_keys_are_normalised(): void {
		$this->assertSame( 'contact_phone', gwc_pp_sanitize_field_key( 'Contact Phone' ) );
		$this->assertSame( 'contact_phone', gwc_pp_sanitize_field_key( 'contact--phone' ) );
		$this->assertSame( 'a_b', gwc_pp_sanitize_field_key( 'a!!!b' ) );
		$this->assertSame( '', gwc_pp_sanitize_field_key( '___' ) );
		$this->assertSame( '', gwc_pp_sanitize_field_key( '   ' ) );
	}

	/* ── Field definitions ───────────────────────────────────────────────── */

	public function test_a_definition_with_no_key_is_refused(): void {
		$this->assertNull( gwc_pp_sanitize_field( array( 'type' => 'text' ) ) );
	}

	public function test_a_definition_with_an_unknown_type_is_refused(): void {
		$this->assertNull(
			gwc_pp_sanitize_field(
				array(
					'key'  => 'phone',
	...
	public function test_fields_come_back_in_the_saved_order(): void {
		...
		gwc_pp_set_field_order( 'clinic', array( 'c', 'a', 'b' ) );

		$this->assertSame( array( 'c', 'a', 'b' ), array_column( gwc_pp_type_fields( 'clinic' ), 'key' ) );
	}
	...
	public function test_an_order_naming_a_field_that_no_longer_exists_is_ignored(): void {
		...
		$this->assertSame( array( 'b', 'a' ), array_column( gwc_pp_type_fields( 'clinic' ), 'key' ) );
	}
	...
	public function test_a_field_missing_from_the_order_is_appended_not_lost(): void {
		...
		$this->assertSame(
			array( 'b', 'a' ),
			array_column( gwc_pp_type_fields( 'clinic' ), 'key' ),
	...
	public function test_reordering_drops_unknown_keys_and_keeps_forgotten_ones(): void {
		...
		gwc_pp_set_field_order( 'clinic', array( 'b', 'ghost' ) );
	...
	public function test_retiring_a_field_keeps_its_definition(): void {
		...
		$this->assertNull( gwc_pp_find_field( 'clinic', 'phone' ) );

		$entry = gwc_pp_type_schema( 'clinic' );
	...
	public function test_re_adding_a_retired_field_takes_it_off_the_retired_list(): void {
		...
		$this->assertSame( 'Phone again', gwc_pp_find_field( 'clinic', 'phone' )['label'] );
	}

	public function test_retiring_something_that_is_not_there_reports_failure(): void {
		$this->assertFalse( gwc_pp_retire_field( 'clinic', 'nothing' ) );
	}
	...
	public function test_a_missing_option_still_yields_a_usable_schema(): void {
		...
		$this->assertSame( array(), gwc_pp_type_fields( 'anything' ) );
	}

	public function test_a_garbage_option_does_not_fatal(): void {
		update_option( 'gwc_pp_schema', 'this is not an array' );
	...
	public function test_a_half_written_type_entry_is_completed(): void {
		update_option( 'gwc_pp_schema', array( 'types' => array( 'clinic' => array( 'fields' => array() ) ) ) );
		gwc_pp_schema_cache( null, true );
	...
	public function test_writing_the_schema_invalidates_the_read_cache(): void {
		$this->put( 'a' );
		$this->assertCount( 1, gwc_pp_type_fields( 'clinic' ) );

		$this->put( 'b' );
		$this->assertCount(
			2,
			gwc_pp_type_fields( 'clinic' ),
	...
	public function test_a_field_whose_type_vanished_is_dropped_from_the_form(): void {
		...
		$this->assertSame( array( 'phone' ), array_column( gwc_pp_type_fields( 'clinic' ), 'key' ) );
	}
}
