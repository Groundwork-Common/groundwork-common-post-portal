...
final class HandoffTest extends TestCase {
	...
	protected function setUp(): void {
		...
		gwcpp_test_post( self::ORG, GWCPP_ORG_TYPE, 'publish', 0, 'Main Street Trust' );

		gwcpp_test_user( self::MEMBER );
		gwcpp_test_user( self::GRANTEE );
		...
		add_user_meta( self::MEMBER, GWCPP_USER_ORG_META, self::ORG );
		gwcpp_add_post_editor( self::GRANTEE, self::POST );
	}
	...
	public function test_handoff_is_refused_when_the_post_type_has_it_switched_off(): void {
		...
		$result = gwcpp_send_handoff( self::POST, self::MEMBER, 'new@example.org' );

		$this->assertTrue( is_wp_error( $result ) );
		$this->assertSame( 'gwcpp_handoff_off', $result->get_error_code() );
	}
	 ...
	 * account stops at gwcpp_handoff_existing, which is only reachable once
	 * membership has passed.
	 */
	public function test_a_member_of_the_organisation_may_hand_over(): void {
		gwcpp_test_user( 11, array( 'editor' ) );

		$result = gwcpp_send_handoff( self::POST, self::MEMBER, 'user11@example.test' );
	...
	public function test_somebody_with_only_a_direct_grant_may_not_hand_over(): void {
		$result = gwcpp_send_handoff( self::POST, self::GRANTEE, 'new@example.org' );
	...
	public function test_a_stranger_may_not_hand_over(): void {
		...
		$result = gwcpp_send_handoff( self::POST, 99, 'new@example.org' );

		$this->assertTrue( is_wp_error( $result ) );
		$this->assertSame( 'gwcpp_handoff_member', $result->get_error_code() );
	}

	public function test_membership_of_a_different_organisation_does_not_count(): void {
		gwcpp_test_post( 72, GWCPP_ORG_TYPE, 'publish', 0, 'Some Other Trust' );
		gwcpp_test_user( 10 );
		add_user_meta( 10, GWCPP_USER_ORG_META, 72 );

		$result = gwcpp_send_handoff( self::POST, 10, 'new@example.org' );
	...
	public function test_a_malformed_address_is_refused_first(): void {
		$result = gwcpp_send_handoff( self::POST, self::MEMBER, 'not-an-address' );

		$this->assertTrue( is_wp_error( $result ) );
		$this->assertSame( 'gwcpp_handoff_email', $result->get_error_code() );
	}

	public function test_a_post_with_no_organisation_has_nothing_to_hand_over(): void {
		gwcpp_test_post( 73, 'clinic', 'publish', 0, 'Unassigned Clinic' );

		$result = gwcpp_send_handoff( 73, self::MEMBER, 'new@example.org' );

		$this->assertTrue( is_wp_error( $result ) );
		$this->assertSame( 'gwcpp_handoff_org', $result->get_error_code() );
	}
}
