...
final class RateLimitTest extends TestCase {
	...
	public function test_the_first_attempts_are_allowed_and_the_fourth_is_not(): void {
		...
		$this->assertFalse( gwc_pp_rate_limited( 'jane@example.org' ) );
		$this->assertFalse( gwc_pp_rate_limited( 'jane@example.org' ) );
		$this->assertFalse( gwc_pp_rate_limited( 'jane@example.org' ) );
		$this->assertTrue( gwc_pp_rate_limited( 'jane@example.org' ) );
	}

	public function test_a_refused_attempt_still_counts(): void {
		for ( $i = 0; $i < 5; $i++ ) {
			gwc_pp_rate_limited( 'jane@example.org' );
		}
	...
	public function test_one_address_does_not_use_up_another_addresses_allowance(): void {
		...
		$this->assertFalse( gwc_pp_rate_limited( 'mark@example.org' ) );
	}

	public function test_the_address_itself_is_never_stored(): void {
		gwc_pp_rate_limited( 'jane@example.org' );

		$stored = wp_json_encode( get_option( 'gwc_pp_rate_limits' ) );
	...
	public function test_the_window_resets_once_it_has_passed(): void {
		...
		update_option( 'gwc_pp_rate_limits', $state );
	...
	public function test_the_site_wide_window_catches_a_spread_out_attempt(): void {
		...
		for ( $i = 0; $i < 30; $i++ ) {
			gwc_pp_rate_limited( 'person' . $i . '@example.org' );
		}

		$this->assertTrue(
			gwc_pp_rate_limited( 'person-new@example.org' ),
	...
	public function test_expired_counters_are_pruned_rather_than_kept_forever(): void {
		...
		gwc_pp_rate_limited( 'someone-else@example.org' );

		$this->assertCount(
			1,
			get_option( 'gwc_pp_rate_limits' )['signin:email'],
	...
	public function test_counters_from_the_old_unprefixed_shape_are_cleared(): void {
		...
		$this->assertArrayNotHasKey( 'email', get_option( 'gwc_pp_rate_limits' ) );
	}

	public function test_an_unparseable_client_address_still_gets_a_counter(): void {
		$_SERVER['REMOTE_ADDR'] = 'not an ip';

		$this->assertSame( 'unknown', gwc_pp_client_ip() );
		$this->assertFalse( gwc_pp_rate_limited( 'jane@example.org' ) );
	}
	...
	public function test_junk_and_bots_are_not_worth_counting(): void {
		$this->assertFalse( gwc_pp_signin_worth_counting( '', '' ), 'An empty form sends nothing.' );
		$this->assertFalse( gwc_pp_signin_worth_counting( '', 'not-an-address' ), 'A malformed address sends nothing.' );
		$this->assertFalse( gwc_pp_signin_worth_counting( '', 'jane@' ), 'Nor does a half-typed one.' );
		$this->assertFalse(
			gwc_pp_signin_worth_counting( 'http://spam.example', 'jane@example.org' ),
	...
	public function test_a_real_request_is_worth_counting(): void {
		$this->assertTrue( gwc_pp_signin_worth_counting( '', 'jane@example.org' ) );
	}
	 ...
	 * this before gwc_pp_rate_limited(), the bug is back and this test will not
	...
	public function test_junk_submissions_do_not_exhaust_the_site_wide_window(): void {
		for ( $i = 0; $i < 60; $i++ ) {
			...
			if ( gwc_pp_signin_worth_counting( $honeypot, $email ) ) {
				gwc_pp_rate_limited( $email );
			}
		}

		$this->assertFalse(
			gwc_pp_rate_limited( 'a-real-partner@example.org' ),
	...
	public function test_password_attempts_do_not_lock_out_magic_links(): void {
		for ( $i = 0; $i < 60; $i++ ) {
			gwc_pp_login_rate_limited( 'someuser' );
		}
	...
	public function test_magic_link_requests_do_not_lock_out_the_password_form(): void {
		...
		$this->assertFalse( gwc_pp_login_rate_limited( 'someuser' ) );
	}
	...
	public function test_handoff_has_its_own_budget_too(): void {
		...
		$this->assertFalse( gwc_pp_handoff_rate_limited( 7 ) );
	}
	...
	public function test_password_guessing_is_refused_after_five_tries(): void {
		...
		$this->assertTrue( gwc_pp_login_rate_limited( 'someuser' ) );
	}
	...
	public function test_one_accounts_guesses_do_not_use_up_anothers(): void {
		...
		$this->assertFalse(
			gwc_pp_login_rate_limited( 'anotheruser' ),
	...
	public function test_the_username_is_never_stored(): void {
		gwc_pp_login_rate_limited( 'jane.the.director' );

		$this->assertStringNotContainsString(
			'jane.the.director',
			(string) wp_json_encode( get_option( 'gwc_pp_rate_limits' ) )
		);
	}
	...
	public function test_handoff_invitations_are_capped_per_user(): void {
		...
		$this->assertTrue( gwc_pp_handoff_rate_limited( 7 ) );
	}

	public function test_a_different_user_gets_their_own_handoff_allowance(): void {
		for ( $i = 0; $i < 6; $i++ ) {
			gwc_pp_handoff_rate_limited( 7 );
		}

		$this->assertFalse( gwc_pp_handoff_rate_limited( 8 ) );
	}

	/* ── Session length ──────────────────────────────────────────────────── */

	public function test_the_session_length_is_clamped_at_both_ends(): void {
		update_option( 'gwc_pp_settings', array( 'session_hours' => 0 ) );
		gwc_pp_settings_cache( null, true );
		$this->assertSame(
			HOUR_IN_SECONDS,
			gwc_pp_session_seconds(),
			'Zero would expire the cookie as it was set, locking everybody out with no error.'
		);

		update_option( 'gwc_pp_settings', array( 'session_hours' => -5 ) );
		gwc_pp_settings_cache( null, true );
		$this->assertSame( HOUR_IN_SECONDS, gwc_pp_session_seconds() );

		update_option( 'gwc_pp_settings', array( 'session_hours' => 99999 ) );
		gwc_pp_settings_cache( null, true );
		$this->assertSame( 720 * HOUR_IN_SECONDS, gwc_pp_session_seconds() );
	}

	public function test_the_default_session_is_short(): void {
		$this->assertSame( 3 * HOUR_IN_SECONDS, gwc_pp_session_seconds() );
	}

	/* ── Type settings ───────────────────────────────────────────────────── */

	public function test_an_unconfigured_post_type_gets_the_conservative_defaults(): void {
		$this->assertTrue( gwc_pp_type_setting( 'anything', 'require_approval' ) );
		$this->assertFalse( gwc_pp_type_setting( 'anything', 'allow_create' ) );
		$this->assertFalse( gwc_pp_type_setting( 'anything', 'author_grant' ) );
	}
	...
	public function test_a_partially_configured_post_type_still_gets_the_defaults(): void {
		...
		$this->assertTrue( gwc_pp_type_setting( 'clinic', 'allow_create' ) );
		$this->assertTrue(
			gwc_pp_type_setting( 'clinic', 'require_approval' ),
