...
final class UnpublishTest extends TestCase {
	...
	public function test_unpublishing_moves_it_to_draft_and_records_who(): void {
		$this->assertTrue( gwc_pp_unpublish_post( self::POST, self::USER ) );

		$this->assertSame( 'draft', get_post( self::POST )->post_status );
		$this->assertSame(
			self::USER,
			(int) get_post_meta( self::POST, '_gwc_pp_unpublished_by', true ),
	...
	public function test_unpublishing_is_refused_when_the_setting_is_off(): void {
		...
		$this->assertFalse( gwc_pp_unpublish_post( self::POST, self::USER ) );
		$this->assertSame( 'publish', get_post( self::POST )->post_status );
	}

	/* ── Putting it back ─────────────────────────────────────────────────────
	 * gwc_pp_republish_post() used to check only that the post was a draft, so a
	...
	public function test_republishing_puts_it_back_and_clears_the_marker(): void {
		gwc_pp_unpublish_post( self::POST, self::USER );

		$this->assertTrue( gwc_pp_republish_post( self::POST ) );
		$this->assertSame( 'publish', get_post( self::POST )->post_status );
		$this->assertSame( '', get_post_meta( self::POST, '_gwc_pp_unpublished_by', true ) );
	}
	...
	public function test_republishing_is_refused_once_the_setting_is_switched_off(): void {
		...
		$this->assertFalse(
			gwc_pp_republish_post( self::POST ),
	...
	public function test_a_draft_no_portal_user_took_down_cannot_be_republished(): void {
		gwc_pp_test_post( 81, 'clinic', 'draft', 0, 'Never Published' );

		$this->assertFalse(
			gwc_pp_republish_post( 81 ),
	...
	public function test_a_draft_staff_re_drafted_by_hand_cannot_be_republished(): void {
		...
		delete_post_meta( self::POST, '_gwc_pp_unpublished_by' );

		$this->assertFalse( gwc_pp_republish_post( self::POST ) );
	}
