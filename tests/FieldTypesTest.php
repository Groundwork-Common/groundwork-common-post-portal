...
final class FieldTypesTest extends TestCase {
	...
	public function test_every_registered_type_satisfies_the_contract(): void {
		foreach ( array_keys( gwcpp_field_types() ) as $slug ) {
			$this->assertNotNull(
				gwcpp_field_type( $slug ),
				$slug . ' is missing one of the callables in GWCPP_TYPE_CONTRACT, so it would be dropped at runtime.'
			);
		}
	}
	...
	public function test_a_type_missing_a_callable_is_dropped_rather_than_half_used(): void {
		...
		$this->assertNotNull( gwcpp_field_type( 'text' ) );
		$this->assertNull( gwcpp_field_type( 'not_a_real_type' ) );
	}
	...
	public function test_an_unknown_type_fails_closed(): void {
		...
		$this->assertSame( '', gwcpp_field_call( $field, 'sanitize', array( '<b>hi</b>', $field ) ) );
		$this->assertTrue( gwcpp_field_call( $field, 'is_empty', array( 'anything', $field ) ) );
		$this->assertSame( '', gwcpp_field_call( $field, 'to_display', array( 'anything', $field ) ) );
	}

	/* ── Scalars ─────────────────────────────────────────────────────────── */

	public function test_text_is_stripped_of_tags(): void {
		$this->assertSame( 'Hello there', gwcpp_sanitize_text( '<b>Hello</b> there' ) );
	}

	public function test_a_script_element_is_removed_contents_and_all(): void {
		$this->assertSame(
			'',
			gwcpp_sanitize_text( '<script>alert(1)</script>' ),
	...
	public function test_an_array_submitted_where_a_scalar_belongs_becomes_empty(): void {
		...
		$this->assertSame( '', gwcpp_sanitize_text( array( 'a', 'b' ) ) );
		$this->assertSame( '', gwcpp_sanitize_number( array( 1 ) ) );
		$this->assertSame( '', gwcpp_sanitize_phone( array( '555' ) ) );
	}

	public function test_a_maxlength_is_applied(): void {
		$field = array( 'settings' => array( 'maxlength' => 5 ) );

		$this->assertSame( 'abcde', gwcpp_sanitize_text( 'abcdefghij', $field ) );
	}

	public function test_zero_is_not_treated_as_empty(): void {
		$this->assertFalse(
			gwcpp_empty_scalar( '0' ),
			'A number field storing zero must not be deleted as though it were blank.'
		);
		$this->assertTrue( gwcpp_empty_scalar( '' ) );
		$this->assertTrue( gwcpp_empty_scalar( '   ' ) );
	}

	public function test_numbers_are_normalised_so_the_diff_does_not_lie(): void {
		$this->assertSame( '7', gwcpp_sanitize_number( '007' ) );
		$this->assertSame( '7', gwcpp_sanitize_number( '7.0' ) );
		$this->assertSame( '', gwcpp_sanitize_number( 'seven' ) );
		$this->assertSame( '-2.5', gwcpp_sanitize_number( '-2.5' ) );
	}
	...
	public function test_number_bounds_are_checked(): void {
		...
		$this->assertSame( '', gwcpp_validate_number( '5', $field ) );
		$this->assertNotSame( '', gwcpp_validate_number( '0', $field ) );
		$this->assertNotSame( '', gwcpp_validate_number( '11', $field ) );
	}

	/* ── URLs ────────────────────────────────────────────────────────────── */

	public function test_a_bare_domain_gets_https_not_http(): void {
		$this->assertSame( 'https://example.org', gwcpp_sanitize_url( 'example.org' ) );
	}

	public function test_a_javascript_url_is_refused(): void {
		$this->assertSame( '', gwcpp_sanitize_url( 'javascript:alert(1)' ) );
	}

	public function test_a_data_url_is_refused(): void {
		$this->assertSame( '', gwcpp_sanitize_url( 'data:text/html;base64,PHNjcmlwdD4=' ) );
	}

	public function test_an_existing_scheme_is_left_alone(): void {
		$this->assertSame( 'http://example.org/x', gwcpp_sanitize_url( 'http://example.org/x' ) );
	}
	...
	public function test_a_typo_that_is_not_a_host_is_refused(): void {
		$this->assertSame( '', gwcpp_sanitize_url( 'wwwexampleorg' ) );
		$this->assertSame( '', gwcpp_sanitize_url( 'n/a' ) );
		$this->assertSame( '', gwcpp_sanitize_url( 'none' ) );
	}
	...
	public function test_a_real_bare_host_still_works(): void {
		$this->assertSame( 'https://example.org', gwcpp_sanitize_url( 'example.org' ) );
		$this->assertSame( 'https://www.example.co.uk/a/b', gwcpp_sanitize_url( 'www.example.co.uk/a/b' ) );
		$this->assertSame( 'https://example.org?a=1', gwcpp_sanitize_url( 'example.org?a=1' ) );
	}

	/* ── Dates ───────────────────────────────────────────────────────────── */

	public function test_a_real_date_survives(): void {
		$this->assertSame( '2026-02-28', gwcpp_sanitize_date( '2026-02-28' ) );
	}

	public function test_an_impossible_date_is_refused_rather_than_shifted(): void {
		$this->assertSame(
			'',
			gwcpp_sanitize_date( '2026-02-31' ),
	...
	public function test_a_date_in_another_format_is_refused(): void {
		$this->assertSame( '', gwcpp_sanitize_date( '28/02/2026' ) );
		$this->assertSame( '', gwcpp_sanitize_date( 'tomorrow' ) );
	}

	/* ── Phone ───────────────────────────────────────────────────────────── */

	public function test_a_phone_number_keeps_its_formatting_and_extension(): void {
		$this->assertSame( '(205) 555-0199 ext 4', gwcpp_sanitize_phone( '(205) 555-0199 ext 4' ) );
		$this->assertSame( '+44 20 7946 0958', gwcpp_sanitize_phone( '+44 20 7946 0958' ) );
	}

	public function test_a_phone_number_needs_seven_digits(): void {
		$this->assertNotSame( '', gwcpp_validate_phone( '555' ) );
		$this->assertSame( '', gwcpp_validate_phone( '5550199' ) );
		$this->assertSame( '', gwcpp_validate_phone( '+44 20 7946 0958' ) );
	}
	...
	public function test_a_choice_not_on_the_list_is_refused(): void {
		...
		$this->assertSame( 'a', gwcpp_sanitize_choice( 'a', $field ) );
		$this->assertSame( '', gwcpp_sanitize_choice( 'administrator', $field ) );
	}
	...
	public function test_multi_choice_drops_unknown_values_and_deduplicates(): void {
		...
		$this->assertSame(
			array( 'a', 'b' ),
			gwcpp_sanitize_multiselect( array( 'b', 'a', 'a', 'nope' ), $field )
		);
	}
	...
	public function test_multi_choice_is_stored_in_schema_order_so_the_diff_is_stable(): void {
		...
		$this->assertSame(
			gwcpp_sanitize_multiselect( array( 'c', 'a' ), $field ),
			gwcpp_sanitize_multiselect( array( 'a', 'c' ), $field ),
	...
	public function test_an_unticked_checkbox_reads_as_off(): void {
		...
		$this->assertSame( '', gwcpp_sanitize_boolean( array( '__present' => '1' ) ) );
		$this->assertSame( '1', gwcpp_sanitize_boolean( array( '__present' => '1', 'value' => '1' ) ) );
	}

	/* ── The choice editor's format ──────────────────────────────────────── */

	public function test_options_parse_from_the_textarea_format(): void {
		$parsed = gwcpp_parse_options( "al|Alabama\nak|Alaska\nArizona" );
	...
	public function test_duplicate_and_blank_option_lines_are_dropped_not_fatal(): void {
		$parsed = gwcpp_parse_options( "a|A\n\n   \na|Again\nb|B" );
	...
	public function test_display_uses_labels_not_stored_values(): void {
		...
		$this->assertSame( 'Alabama', gwcpp_display_choice( 'al', $field ) );
	}
