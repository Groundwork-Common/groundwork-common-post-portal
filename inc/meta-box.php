...
function gwcpp_render_members_meta_box( WP_Post $post ): void {
	...
	if ( ! $members ) {
		...
		foreach ( $members as $user ) {
			...
			$last = (int) get_user_meta( $user->ID, 'gwcpp_last_login', true );
			printf(
				'<td>%s</td>',
				esc_html(
					$last > 0
...
function gwcpp_save_access_meta( $post_id, $post ): void {
	...
	gwcpp_set_post_org( (int) $post_id, isset( $_POST['gwcpp_org'] ) ? (int) $_POST['gwcpp_org'] : 0 );
}
...
add_action(
	...
	static function ( $login, $user ): void {
		...
		if ( $user instanceof WP_User && gwcpp_user_is_portal_user( $user->ID ) ) {
			update_user_meta( $user->ID, 'gwcpp_last_login', time() );
		}
	},
	10,
	2
